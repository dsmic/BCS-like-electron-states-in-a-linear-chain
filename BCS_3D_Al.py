...
UseSymmetryForDiff_q = True # this means the differentiation coresponding to all atom positions is the same, only not the number!
...
CHAIN_RS_PARM = 10 # Gaus width of real space amplitude
...
DIST_UNITCELL = 7.63075       # Aluminium parameters from quantum espresso (http://quantum-espresso.org/)
...
# create all atoms
UNIT_CELL_ATOMS = [[0,0,0],[0.5,0.5,0],[0.5,0,0.5],[0,0.5,0.5]]
...
MASS_ATOM = 27 * 1836 # aluminum times the proton-electron mass ratio (we only support one atom type at the moment)
...
VP_FACT = 0.01 # this is a harmonic force, should become very small later, as the force between the atoms will result from electron phonon coupling, Only avoids simultan movement
        ...
        for i1 in range(-UNIT_CELL_OVERLAP, NUM_UNIT_CELLS[1]+UNIT_CELL_OVERLAP):
            ...
            for i2 in range(-UNIT_CELL_OVERLAP, NUM_UNIT_CELLS[2]+UNIT_CELL_OVERLAP):
                ...
                for CELL in UNIT_CELL_ATOMS:
                    ALL_ATOMS.append([i0+CELL[0],i1+CELL[1],i2+CELL[2], i0_mod*NUM_UNIT_CELLS[1]*NUM_UNIT_CELLS[2]*l + i1_mod*NUM_UNIT_CELLS[2]*l + i2_mod * l + c])                                                        # the last indicates the atom in the supercell
                    c+=1
...
UNIT_CELL_OVERLAP = 0
ONLY_SUPERCELL_ATOMS = numpy.array([DIST_UNITCELL, DIST_UNITCELL, DIST_UNITCELL, 1]) * get_Atoms()
...
EL_WV_LIST = numpy.concatenate((numpy.array(EL_WV_LIST_DEF)[:,:3] * (2*math.pi/DIST_UNITCELL/numpy.array(NUM_UNIT_CELLS)), numpy.array(EL_WV_LIST_DEF)[:,3:]), axis=1).tolist()

print('EL_WV_LIST', EL_WV_LIST, "def", EL_WV_LIST_DEF, "EL_PARAM_STEPS", EL_PARAM_STEPS)

INTEGRATE_ELECTRONS_VOLUME = [[0, NUM_UNIT_CELLS[0]*DIST_UNITCELL], [0, NUM_UNIT_CELLS[1]*DIST_UNITCELL], NUM_UNIT_CELLS[2]*[0,DIST_UNITCELL]]

MEAN_FIELD_DIST = DIST_UNITCELL
...
MEAN_FACT = len(ONLY_SUPERCELL_ATOMS) / 2 * VALENCE_ELECTRONS    

UNIT_CELL_OVERLAP = UNIT_CELL_OVERLAP_OLD
ALL_ATOMS = get_Atoms()
ALL_ATOMS = numpy.array([DIST_UNITCELL, DIST_UNITCELL, DIST_UNITCELL, 1]) * ALL_ATOMS            # the last is the variable number of the position variables of the nuclei in the integration range
...
RECIPROCAL_LENGTH = 2*pi/(NUM_UNIT_CELLS[0] * DIST_UNITCELL)   # the reciprocal length for electrons is only in the x direction
...
if DEBUG_PLOTTING:
    N_ELECTRONS = 1
    X = numpy.linspace(0,NUM_UNIT_CELLS[0]*DIST_UNITCELL,50)

    Y = numpy.linspace(0,NUM_UNIT_CELLS[1]*DIST_UNITCELL,50)
    ...
    def v_el_lamb_vec(X, Y, H):
        ...
        for i in range(X.shape[0]):
            ...
            for j in range(X.shape[1]):
                ...
                # inp_tmp.append(2)
                ret1.append(v_el_lamb(numpy.concatenate((inp_nump, numpy.array([X[i,j], Y[i,j], H]))), numpy.array([1])))
            ret.append(ret1)
        return numpy.array(ret)
            

    Z = v_el_lamb_vec(X, Y, DIST_UNITCELL/4)
    Z1 = v_el_lamb_vec(X, Y, DIST_UNITCELL/8)
    Z2 = v_el_lamb_vec(X, Y, 0)
...
def pot_nuclei_lamb(_, efact): # Calculates the potential of the 000 nuclei in all other nucleis
    ...
    dd = dd**0.5
    dd = PseudoPotential_for_Al(dd)
    dd = dd.sum() - PseudoPotential_for_Al(0) # correct nucleis own potential
    return -ELECTRONS_IN_CALCULATION * dd
...
if DEBUG_PLOTTING:
    koef_tmp = numpy.array([1,0,1,0,1,0,1])
    ...
    def phi_q_el_one_vec(X, Y):
        ...
        for i in range(X.shape[0]):
            ret1 = []
            for j in range(X.shape[1]):
                ret1.append(N(phi_q_el_one(numpy.concatenate((inp_nump, numpy.array([X[i,j], Y[i,j], 0]))), 0, [0,0,0], koef_tmp) * conjugate(phi_q_el_one(numpy.concatenate((inp_nump, numpy.array([X[i,j], Y[i,j], 0]))), 0, [0,0,0], koef_tmp))))
            ret.append(ret1)
        return numpy.array(ret)


    Z = phi_q_el_one_vec(X, Y)
...
def diff_q(xp, ee, efact):
    if UseSymmetryForDiff_q: 
        ...
        if DoSimplifiedDiffQ:
            return ret * phi_q_el_slater(xp, efact[int(NUM_UNIT_CELLS[0]/2)+1:])
        else:
            return ret
    ...
    if DoDiffMultiprocessing:
        ...
        diffpool.join()
        if DoSimplifiedDiffQ:
            return res * phi_q_el_slater(xp, efact[int(NUM_UNIT_CELLS[0]/2)+1:])
        else:
            return res 
...
def phi_q(q, ee, efact):
    if not CHAIN_WF_ONLY_REAL_SPACE_NO_EXCITATIONS:
        ...
        for k in range(0,NUM_3D_ATOMS_IN_INTEGRATION_RANGE):                 # all dimensions are used
            prod *= exp(-efact[1] * q[k]**2)              # efact[1], because in the Q space no efact[0] is used, therefore it is not included in the parameters
        return prod

def phi_full(q, ee, efact):
    return phi_q(q, ee, efact) * phi_q_el_slater(q, efact[int(NUM_UNIT_CELLS[0]/2)+1:])
...
efact_symbols = list(symbols(''.join(' efact'+str(i) for i in range(int(NUM_UNIT_CELLS[0]/2)+1 + NUM_KOEFS*2 + 1)), real=True))
...
if DO_MEAN:
    phi_q_one_dense_lamb = lambdify([xp, efact], conjugate(phi_q_el_one(xp, N_ELECTRONS, [0,0,0], efact[int(NUM_UNIT_CELLS[0]/2)+1:]) * phi_q_el_one(xp, N_ELECTRONS, [0,0,0], efact[int(NUM_UNIT_CELLS[0]/2)+1:])) , cse=True, modules=['numpy'])    # the last is taken
    if DoCompile:
...
tttt = diff_q(xp,ee1,efact)
print(time.strftime("diff: %H:%M:%S", time.localtime()))
diff_q_lamb  = lambdify([xp, ee1, efact], tttt, cse=True, modules=['numpy'])
print(time.strftime("lamb: %H:%M:%S", time.localtime()))
if DoCompile:
    diff_q_lamb = numba.jit(diff_q_lamb, nogil=True)
...
def integrand_H_numba(xp, ee1, ee2, efact, norm_density):
    return numpy.conjugate(phi_full_lamb(xp,ee1,efact)) * ((v_el_lamb(xp,efact)+v_lamb(xp)+v_el_mean_lamb(xp,efact)/norm_density) * phi_full_lamb(xp,ee2,efact) - 1.0 / MASS_ATOM * diff_q_lamb(xp, ee2, efact) - diff_q_el_lamb(xp, ee2, efact))
#@numba.jit(nopython=True, nogil=True)
...
wf_params = []
for i in range(int(NUM_UNIT_CELLS[0]/2)+1):
...
while len(excitations) < int(NUM_UNIT_CELLS[0]/2)+1:
...
if CHAIN_WF_ONLY_REAL_SPACE_NO_EXCITATIONS:
    wf_params = [0] + [CHAIN_RS_PARM] + [0]*int(NUM_UNIT_CELLS[0]/2-1)
...
if EVALS > 0:
    ...
    start_steps = [2]*(int(NUM_UNIT_CELLS[0]/2)) + ([0] + [phase_steps] + [EL_PARAM_STEPS]*(NUM_KOEFS*2-2)) + [DO_RELAX]
    if CHAIN_WF_ONLY_REAL_SPACE_NO_EXCITATIONS:
        start_steps = [CHAIN_RS_PARM / 10] + [0]*int(NUM_UNIT_CELLS[0]/2-1) + ([0] + [phase_steps] + [EL_PARAM_STEPS]*(NUM_KOEFS*2-2)) + [DO_RELAX]
...
if DO_AFTER_OPTIMIZATION_EVAL is not None:
    ...
    if DO_AFTER_SIMULTAN is not None:
